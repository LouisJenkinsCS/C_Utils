...
int main(void) {
	...
	DS_Iterator_t *it = c_utils_list_iterator(list);
	MU_ASSERT(*(int *)c_utils_list_get(list, runs/2) == *(int *)array[runs/2], logger, "Unable to get the right element from the list!");
	...
	MU_ASSERT(c_utils_list_remove_item(list, array[runs/2], NULL), logger, "Unable to remove item from the list!");
	int *item = NULL;
