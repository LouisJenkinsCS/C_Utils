...
int main(void) {
	srand(time(NULL));
	logger = calloc(1, sizeof(struct c_utils_logger));
	...
	C_UTILS_LOG_INFO(logger, "Testing the Array-To-Linked_List functionality and sorting!\n");
	...
	C_UTILS_LOG_INFO(logger, "Amount of Runs: %d; Total time is: %s\n", runs, total_time);
	free(total_time);
	Timer_Destroy(timer); 
