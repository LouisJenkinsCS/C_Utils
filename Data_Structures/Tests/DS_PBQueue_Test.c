...
void print_value(void *number) {
	C_UTILS_LOG_VERBOSE(logger, "Queue has value: %d\n", *(int *)number);
	free(number);
}
...
void *producer_timeout_thread(void *args) {
	...
	*i = 0;
	C_UTILS_LOG_VERBOSE(logger, "Producer: sleeping for 10 seconds...\n");
	sleep(10);
	C_UTILS_LOG_VERBOSE(logger, "Producer: woke up, Enqueueing now!\n");
	DS_PBQueue_enqueue(queue, i, 5);
	C_UTILS_LOG_VERBOSE(logger, "Producer: sleeping for 10 seconds...\n");
	sleep(10);
	C_UTILS_LOG_VERBOSE(logger, "Producer: woke up, timed enqueueing %d times for 10 second timeout!\n", max_size);
	int j = 0;
	for (;j <= max_size; j++) {
		C_UTILS_LOG_VERBOSE(logger, "Producer: starting Enqueue %d\n", j);
