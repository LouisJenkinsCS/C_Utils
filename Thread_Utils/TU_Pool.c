...
static TP_Worker_t *Get_Self(TU_Pool_t *tp){
...
static void Set_Task_Priority(TP_Task_t *task, int flags){
	if(MU_FLAG_GET(flags, TU_LOWEST_PRIORITY)) task->priority = TU_LOWEST;
	else if(MU_FLAG_GET(flags, TU_LOW_PRIORITY)) task->priority = TU_LOW;
	else if(MU_FLAG_GET(flags, TU_HIGH_PRIORITY)) task->priority = TU_HIGH;
	else if(MU_FLAG_GET(flags, TU_HIGHEST_PRIORITY)) task->priority = TU_HIGHEST;
	else task->priority = TU_MEDIUM;

}

static char *Get_Task_Priority(TP_Task_t *task){
	if(task->priority == TU_LOWEST) return "Lowest";
	else if(task->priority == TU_LOW) return "Low";
	else if(task->priority == TU_MEDIUM) return "Medium";
	else if(task->priority == TU_HIGH) return "High";
	else if(task->priority == TU_HIGHEST) return "Highest";
	else{
