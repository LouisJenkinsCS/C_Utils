...
static void *_acquire_scoped_lock_rwlock_write(struct c_utils_scoped_lock *s_lock) {
   int errcode = pthread_rwlock_wrlock(s_lock->lock);
   if (errcode)
      C_UTILS_LOG_LOCK(s_lock, C_UTILS_LOG_LEVEL_ERROR, "pthread_rwlock_wrlock: \"%s\"", strerror(errcode));
   else
      C_UTILS_LOG_LOCK(s_lock, C_UTILS_LOG_LEVEL_TRACE, "Obtained lock...");
...
static void *_acquire_scoped_lock_rwlock_read(struct c_utils_scoped_lock *s_lock) {
   int errcode = pthread_rwlock_rdlock(s_lock->lock);
   if (errcode)
      C_UTILS_LOG_LOCK(s_lock, C_UTILS_LOG_LEVEL_ERROR, "pthread_rwlock_rdlock: \"%s\"", strerror(errcode));
   else
      C_UTILS_LOG_LOCK(s_lock, C_UTILS_LOG_LEVEL_TRACE, "Obtained lock...");
