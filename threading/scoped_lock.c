...
#define C_UTILS_LOG_LOCK(lock, level, format, ...) c_utils_logger_log(lock->logger, level, "LOCK", format, NULL, lock->info.file, lock->info.line, lock->info.function, ##__VA_ARGS__)
...
static void *_bad_acquire1(struct c_utils_scoped_lock *s_lock){
   C_UTILS_LOG_LOCK(s_lock, LOG_LEVEL_ERROR, "The underlying lock does not support a secondary locking mechanism.");
}
...
static void *_destroy_scoped_lock_mutex(struct c_utils_scoped_lock *s_lock){
   ...
   if(errcode){
      LOG_ERROR(s_lock->logger, "pthread_mutex_destroy: \"%s\"", strerror(errcode));
   }
   free(s_lock->lock);
   return s_lock;
}
...
static void *_acquire_scoped_lock_mutex(struct c_utils_scoped_lock *s_lock){
   ...
   if(errcode){
      C_UTILS_LOG_LOCK(s_lock, LOG_LEVEL_ERROR, "pthread_mutex_lock: \"%s\"", strerror(errcode));
   } else {
      C_UTILS_LOG_LOCK(s_lock, LOG_LEVEL_TRACE, "Obtained lock...");
   }
   return s_lock;
}

static void *_release_scoped_lock_mutex(struct c_utils_scoped_lock *s_lock){
   C_UTILS_LOG_LOCK(s_lock, LOG_LEVEL_TRACE, "Releasing lock...");
   int errcode = pthread_mutex_unlock(s_lock->lock);
   if(errcode){
      C_UTILS_LOG_LOCK(s_lock, LOG_LEVEL_ERROR, "pthread_mutex_unlock: \"%s\"", strerror(errcode));
   }
   return s_lock;
}
...
c_utils_scoped_lock *c_utils_scoped_lock_mutex(pthread_mutex_t *lock, struct c_utils_logger *logger){
   ...
   if(!lock){
      LOG_INFO(logger, "Passed lock was NULL, all locking operations set to a NO-OP!");
...
static void *_destroy_scoped_lock_spinlock(struct c_utils_scoped_lock *s_lock){
   ...
   if(errcode){
      C_UTILS_LOG_LOCK(s_lock, LOG_LEVEL_ERROR, "pthread_spin_destroy: \"%s\"", strerror(errcode));
   }
   free(s_lock->lock);
   return s_lock;
}
...
static void *_acquire_scoped_lock_spinlock(struct c_utils_scoped_lock *s_lock){
   ...
   if(errcode){
      C_UTILS_LOG_LOCK(s_lock, LOG_LEVEL_ERROR, "pthread_spin_lock: \"%s\"", strerror(errcode));
   } else {
      C_UTILS_LOG_LOCK(s_lock, LOG_LEVEL_TRACE, "Obtained lock...");
   }
   return s_lock;
...
static void *_release_scoped_lock_spinlock(struct c_utils_scoped_lock *s_lock){
   ...
   if(errcode){
      C_UTILS_LOG_LOCK(s_lock, LOG_LEVEL_ERROR, "pthread_spin_unlock: \"%s\"", strerror(errcode));
   }
   return s_lock;
}
