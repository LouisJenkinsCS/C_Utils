...
static void auto_reset_handler(c_utils_event_t *event, unsigned int thread_id){
	if(MU_FLAG_GET(event->flags, EVENT_AUTO_RESET_ON_LAST)){
	...
	} else if(MU_FLAG_GET(event->flags, EVENT_AUTO_RESET)){
...
struct c_utils_event_t *c_utils_event_create(const char *event_name, MU_Logger_t *logger, unsigned int flags){
	...
	c_utils_event_t *event = calloc(1, sizeof(c_utils_event_t));
	if(!event){
	...
	event->signaled = ATOMIC_VAR_INIT(MU_FLAG_GET(flags, EVENT_SIGNALED_BY_DEFAULT));
	...
	while(atomic_load(&event->signaled) == false){
		...
		if(errcode){
			...
			if(errcode != ETIMEDOUT){
				...
				if(MU_FLAG_GET(event->flags, EVENT_SIGNAL_ON_TIMEOUT)){
...
bool c_utils_event_signal(struct c_utils_event_t *event, unsigned int thread_id){
	if(!event) return false;
	if(atomic_load(&event->signaled) == true){
		return true;
	}
