...
static size_t timed_receive(int sockfd, void *buffer, size_t buf_size, long long int timeout, int flags, struct c_utils_logger *logger){
   ...
   if(received <= 0){
      if(!received) LOG_INFO(logger, "select: 'Timed out!'");
   ...
   if(received <= 0){
      if(!received) LOG_INFO(logger, "recv: 'Disconnected from the stream!'");
      else LOG_ERROR(logger, "recv: '%s'", strerror(errno));
      return 0;
   }
...
static size_t send_all(int sockfd, const void *buffer, size_t buf_size, long long int timeout, int flags, struct c_utils_logger *logger){
   ...
   while(buf_size > total_sent){
      ...
      if(sent <= 0){
         if(!sent) LOG_INFO(logger, "select: 'Timed out!'");
         else LOG_ERROR(logger, "select: '%s'", strerror(errno));
         break;
      }
      ...
      if(sent <= 0){
         if(!sent) LOG_INFO(logger, "send: 'Disconnected from the stream'");
         else LOG_ERROR(logger, "send: '%s'", strerror(errno));
         break;
      }
...
struct c_utils_connection *c_utils_connection_create(bool init_locks, struct c_utils_logger *logger){
	...
	if(init_locks){
		...
		if((retval = pthread_rwlock_init(lock, NULL)) < 0){
			LOG_ERROR(logger, "pthread_rwlock_init: '%s'", strerror(retval));
			goto error;
			return NULL;
		}
	...
	if(!conn->lock){
		LOG_ERROR(logger, "SCOPED_LOCK_FROM: 'Unable to create scoped lock from rwlock!");
		goto error;
	}
...
size_t c_utils_connection_send(struct c_utils_connection *conn, const void *buffer, size_t buf_size, long long int timeout, int flags){
	ARG_CHECK(conn->logger, 0, conn, buffer, buf_size > 0);
	
	size_t total_sent = 0;
	SCOPED_LOCK1(conn->lock) {
		total_sent = send_all(conn->sockfd, buffer, buf_size, timeout, flags, conn->logger);
	}
	return total_sent;
}
...
size_t c_utils_connection_receive(struct c_utils_connection *conn, void *buffer, size_t buf_size, long long int timeout, int flags){
	...
	SCOPED_LOCK1(conn->lock) {
		amount_received = timed_receive(conn->sockfd, buffer, buf_size, timeout, flags, conn->logger);
	}
...
size_t c_utils_connection_send_file(struct c_utils_connection *conn, FILE *file, long long int timeout, int flags){
	ARG_CHECK(conn->logger, 0, conn, file);
	...
	SCOPED_LOCK1(conn->lock) {
		...
		if(file_fd == -1){
			LOG_WARNING(conn->logger, "fileno: '%s'", strerror(errno));
			return 0;
		}

		if(fstat(file_fd, &file_stats) == -1){
			LOG_WARNING(conn->logger, "fstat: '%s'", strerror(errno));
			return 0;
		}
		...
		while(buf_read > 0){
			if(send_all(conn->sockfd, buf, buf_read, timeout, flags, conn->logger) != buf_read){
				LOG_WARNING(conn->logger, "c_utils_connection_send_file->send_all: 'Was unable to send all of message to %s'", conn->ip_addr);
		...
		if(total_sent != file_size){
			LOG_WARNING(conn->logger, "Was unable to send all of file to %s!File Size is %zu, but could only send %zu!",
...
size_t c_utils_connection_receive_file(struct c_utils_connection *conn, FILE *file, long long int timeout, int flags) {
	...
	SCOPED_LOCK1(conn->lock) {
		...
		while((received = timed_receive(conn->sockfd, buf, buf_size, timeout, flags, conn->logger)) > 0){
			...
			if(written != received){
				LOG_ERROR(conn->logger, "fwrite: 'Written only %zu bytes, expected %zu bytes!\n%s'", written, received, strerror(errno));
...
int c_utils_connection_select(struct c_utils_connection ***receivers, size_t *r_size, struct c_utils_connection ***senders, size_t *s_size,
	...
	/* 
		...
		and if not, logs it appropriately. This kind of check isn't generic enough to use ARG_CHECK for, as it evaluates them individually,
	...
	*/
	if((!receivers || !r_size || !*r_size) && (!senders || !s_size || *s_size)){
		LOG_ERROR(logger, "Invalid Arguments: 'Receivers: %s;Receiver Size_ptr: %s;Receiver Size > 0: %s;"
	...
	*/
	...
	if(!can_receive && !can_send){
		LOG_WARNING(logger, "Was unable to find a valid receiver or sender connection!");
		goto error;
	}
	...
	*/
	...
	if(are_ready <= 0){
		if(!are_ready) LOG_INFO(logger, "select: 'Timed out!'");
		else LOG_WARNING(logger, "select: '%s'", strerror(errno));
		goto error;
	}
...
struct c_utils_connection *c_utils_connection_reuse(struct c_utils_connection **connections, size_t size, int sockfd,
	...
	ARG_CHECK(logger, NULL, connections, size > 0, sockfd > -1, port > 0, ip_addr);
...
bool c_utils_connection_set_sockfd(struct c_utils_connection *conn, int sockfd){
	ARG_CHECK(conn->logger, false, conn);

	SCOPED_LOCK0(conn->logger) conn->sockfd = sockfd;
	return true;
}

int c_utils_connection_get_sockfd(struct c_utils_connection *conn){
	ARG_CHECK(conn->logger, -1, conn);
...
bool c_utils_connection_set_logger(struct c_utils_connection *conn, struct c_utils_logger *logger){
	ARG_CHECK(logger, false, conn);

	SCOPED_LOCK0(conn->lock) conn->logger = logger;
	return true;
}
...
bool c_utils_connection_init(struct c_utils_connection *conn, int sockfd, unsigned int port, const char *ip_addr, struct c_utils_logger *logger){
	ARG_CHECK(logger, false, conn, sockfd > -1, port > 0, ip_addr);

	SCOPED_LOCK0(conn->lock) {
		if(conn->in_use){
			LOG_INFO(logger, "c_utils_connection_init: 'Connection already in use!'");
			return false;
		}
...
bool c_utils_connection_disconnect(struct c_utils_connection *conn){
	...
	SCOPED_LOCK0(conn->lock) {
		...
		if(has_closed == -1) LOG_WARNING(conn->logger, "close: '%s'", strerror(errno));
